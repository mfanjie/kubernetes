...
func (j *testJig) createIngress(manifestPath, ns string, ingAnnotations map[string]string) {
	...
	var err error
	err = j.client.Post().Namespace(ns).Resource(ingressResourceName).Name(j.ing.Name).Body(j.ing).Do().Error()
	ExpectNoError(err)
}
...
func (j *testJig) deleteIngress() {
	...
	ExpectNoError(j.client.Delete().Namespace(j.ing.Namespace).Resource(ingressResourceName).Name(j.ing.Name).Do().Error())
}
